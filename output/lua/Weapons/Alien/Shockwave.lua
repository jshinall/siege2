...
function Shockwave:UpdateShockwave(deltaTime)
    ...
    local tracey = Shared.TraceRay(self:GetOrigin(), newPosition, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FrontDoor")) 
    local tracex = Shared.TraceRay(self:GetOrigin(), newPosition, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncMoveable")) 
    local tracet = Shared.TraceRay(self:GetOrigin(), newPosition, CollisionRep.Move, PhysicsMask.LogicBreakable, EntityFilterAllButIsa("LogicBreakable")) 
     if (newPosition - self.endPoint):GetLength() < 0.1 or (tracey.entity ~= nil and tracey.entity:isa("FrontDoor")) or (tracet.entity ~= nil and tracet.entity:isa("LogicBreakable")) or (tracex.entity ~= nil and tracex.entity:isa("FuncMoveable")) then
        DestroyShockwave(self)
     else
