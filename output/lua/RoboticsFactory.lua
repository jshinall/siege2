...
function RoboticsFactory:GetTechButtons(techId)
    ...
    local techButtons = {  kTechId.ARC, kTechId.MAC, kTechId.MacWeldMacs, kTechId.None, 
...
function RoboticsFactory:GetArcsAmount()
    local arcs = 0
        for index, mac in ientitylist(Shared.GetEntitiesWithClassname("ARC")) do
                arcs = arcs + 1
         end
    return  arcs
end
  ...
  function RoboticsFactory:OnUpdate()
   ...
   if self.automaticspawningmac then
    ...
    if self.automaticspawningarc then
        if self:GetTeam():GetTeamResources() >= kARCCost and ( kMaxSupply - GetSupplyUsedByTeam(1) >= LookupTechData(kTechId.ARC, kTechDataSupply, 0)) and self.deployed and GetIsUnitActive(self) and self:GetResearchProgress() == 0 and not self.open and self:GetArcsAmount() <= 8 - 1 then
