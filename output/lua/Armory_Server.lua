...
function Armory:GetShouldResupplyPlayer(player)
    ...
    if (player:GetHealth() < player:GetMaxHealth()) or (player:GetArmor() < player:GetMaxArmor() and self.healarmor == true ) then
        inNeed = true
    else
...
function Armory:ResupplyPlayer(player)
    ...
    if (player:GetHealth() < player:GetMaxHealth()) or (player:GetArmor() < player:GetMaxArmor() and self.healarmor == true ) then
        ...
        player:AddHealth(Armory.kHealAmount * scale, false, not self.healarmor, nil, nil, true)
...
function Armory:OnResearchComplete(researchId)
    ...
    self.healarmor = true
        ...
        if researchNode then     
            ...
            techTree:SetTechNodeChanged(researchNode, string.format("researchProgress = %.2f", self.researchProgress))
            researchNode:SetResearched(true)
            techTree:QueueOnResearchComplete(kTechId.ArmoryArmor, self)
