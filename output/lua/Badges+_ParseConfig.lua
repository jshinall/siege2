...
do	
	local function LoadConfigFileCredits(fileName)
		Shared.Message( StringFormat( "Loading Badge config://%s", fileName ))
		local openedFile = io.open( StringFormat( "config://shine/plugins/%s", fileName ), "r" )
		if openedFile then
	...
	local creditusers = ParseJSONStruct( LoadConfigFileCredits( "credits.json" ))
	if creditusers.Users then
		for _, user in pairs(creditusers.Users) do
				local userId = user.id
				GiveBadge( userId, star )
		end 
    end
