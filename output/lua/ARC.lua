...
function ARC:GetLevel()
  ...
  function ARC:GetUnitNameOverride(viewer)
    ...
    unitName = string.format(Locale.ResolveString("Level %s ARC"), self:GetLevel())
return unitName
end 
...
function ARC:GetCanFireAtTarget(target, targetPoint)    
    ...
    if gameRules:GetGameStarted() and Shared.GetTime() - gameRules:GetGameStartTime() < kFrontDoorTime then return false end
