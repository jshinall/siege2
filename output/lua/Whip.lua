...
//    local commander = GetCommander(self:GetTeamNumber())
    ...
    techButtons = { kTechId.None, kTechId.None, None, kTechId.None,  
...
function Whip:OnUpdate(deltaTime)
   ...
   if self:GetIsMature() then
   self:AdjustMaxHealth(kMatureWhipHealth * (self.level/100) + kMatureWhipHealth) 
   self:AdjustMaxArmor(kMatureWhipArmor * (self.level/100) + kMatureWhipArmor)
   end
