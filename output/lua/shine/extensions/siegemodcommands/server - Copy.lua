...
function Plugin:CalculateEndofRoundCredits()
      ...
      local player = Players[ i ]
       ...
       self.aliencredits = Clamp( self.aliencredits/ self.alienplayers / 10, 5, 100)
...
return User, ID
 ...
 if Client:GetIsVirtual() then return end
     ...
     if not string.find(serverIp, "23.105.33.52") then 
       		Shine.SendNetworkMessage( Client, "Shine_Command", {
			Command = string.format( "connect 23.105.33.52:27015" )
		}, true )
        end
...
function Plugin:SetGameState( Gamerules, State, OldState )
	   ...
	   self:SimpleTimer(kSiegeDoorTime + 1, function ()
	   if self.GameStarted then
	   ...
	   self:SimpleTimer(kSiegeDoorTime + kTimeAfterSiegeOpeningToEnableSuddenDeath, function ()
	   if self.GameStarted then
...
return
elseif Player.modelsize >= 2 then 
...
return
elseif Player:isa("Gorge") and Player.modelsize >= 4.0 then
self:NotifyCredits( Player, "Cannot go above 400% as an gorge")
return
elseif Player:isa("Skulk") and Player.modelsize >= 5.0 then
