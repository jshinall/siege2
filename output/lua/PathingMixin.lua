...
function PathingMixin:CheckTarget(endPoint)
   ...
   local tracex = Shared.TraceRay(self:GetOrigin(), endPoint, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("SiegeDoor"))                                                                    
   local trace = Shared.TraceRay(self:GetOrigin(), endPoint, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FrontDoor"))  
   local tracey = Shared.TraceRay(self:GetOrigin(), endPoint, CollisionRep.Move, PhysicsMask.LogicBreakable, EntityFilterAllButIsa("LogicBreakable"))  
   ...
   if (trace.entity ~= nil and trace.entity:isa("FrontDoor")) or (tracey.entity ~= nil and tracey.entity:isa("LogicBreakable")) or (tracex.entity ~= nil and tracex.entity:isa("SiegeDoor")) then //or (tracez.entity ~= nil and tracez.entity:isa("FuncDoor") and tracez.entity:GetState() == Door.kState.Locked)then 
   return false
   end    
   local tracex = Shared.TraceRay(self:GetOrigin(), endPoint, CollisionRep.Move, PhysicsMask.FuncMoveable, EntityFilterAllButIsa("FuncMoveable"))    
   if Shared.GetMapName() == "ns2_tram_siege" and (tracex.entity ~= nil and tracex.entity:isa("FuncMoveable")) then
    return false
  end
  return true
