...
{      
    ...
    rtdmaxarmoradjustment = "float (-50 to 50 by 1)",
...
function Marine:GetArmorAmount(armorLevels)
    ...
    local amount = ( (Marine.kBaseArmor + armorLevels * Marine.kArmorPerUpgradeLevel) * self.modelsize )
    return amount * (self.rtdmaxarmoradjustment/100) + amount
...
function Marine:GetIsStunAllowed()
    return not self.timeLastStun or self.timeLastStun + kDisruptMarineTimeout < Shared.GetTime() and not self:GetIsVortexed() and not self.spawnprotection
end
