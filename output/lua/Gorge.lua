...
function Gorge:GetCanWallJump()
    ...
    if wallWalkNormal and GetHasTech(self, kTechId.SpiderGorge) then
...
function Gorge:GetMaxSpeed(possible)
    ...
    if possible then return kMaxGroundSpeed * size end
...
function Gorge:GetCanBeUsed(player, useSuccessTable)
    useSuccessTable.useSuccess = useSuccessTable.useSuccess and player:isa("Lerk")    
end
function Gorge:OnUse(player, elapsedTime, useSuccessTable)
local parent = self:GetParent()
if parent == nil then
    return self:SetParent(player), self:SetAttachPoint(Gorge.kLerkAttachPoint) 
elseif parent == player then
return self:SetParent(nil)
end
end
Onupdate if parent ()~= nil then setorigin parent
the parent of the gorge parent aka grandaddy
Gorge:GetParent():GetParent()
*/
...
function Gorge:PreUpdateMove(input, runningPrediction)
    ...
    if self.wallWalking then
        ...
        if goal ~= nil and self:HasEnoughEnergyToWallFuck() and GetHasTech(self, kTechId.SpiderGorge) then //and not self:GetEnergy() < kWallWalkEnergyCost then 
